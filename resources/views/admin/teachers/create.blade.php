...
@section('title', 'Create Teacher - NutriMind Admin')

@section('content')
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-text-primary">Create Teacher</h1>
        <p class="mt-1 text-sm text-text-secondary">Add a new teacher account to NutriMind.</p>
    </div>
    ...
    @if (session('success'))
        <div class="mb-6 rounded-lg border border-brand/20 bg-brand/5 p-5">
            ...
            <div class="mt-4 rounded-md bg-surface-card p-4">
                <p class="text-sm text-text-secondary">Teacher credentials — share these securely:</p>
                <div class="mt-3 space-y-2">
                    <div class="flex items-center justify-between">
                        <span class="text-xs uppercase tracking-wider text-text-muted">Name</span>
                        <span class="font-mono text-sm text-text-primary">{{ session('teacher_name') }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-xs uppercase tracking-wider text-text-muted">Username</span>
                        <span class="font-mono text-sm text-text-primary">{{ session('teacher_username') }}</span>
                    </div>
                    <div class="flex items-center justify-between">
