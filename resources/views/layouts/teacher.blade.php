...
<head>
    ...
    <title>@yield('title', 'NutriMind - Teacher')</title>
        ...
        <aside class="fixed inset-y-0 left-0 z-30 flex w-60 flex-col bg-surface-card">
            ...
            <div class="flex h-16 items-center gap-3 px-5">
                <div class="flex h-8 w-8 items-center justify-center rounded-full bg-brand">
                    <svg class="h-4 w-4 text-black" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M4.26 10.147a60.438 60.438 0 00-.491 6.347A48.62 48.62 0 0112 20.904a48.62 48.62 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.636 50.636 0 00-2.658-.813A59.906 59.906 0 0112 3.493a59.903 59.903 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0112 13.489a50.702 50.702 0 017.74-3.342M6.75 15a.75.75 0 100-1.5.75.75 0 000 1.5zm0 0v-3.675A55.378 55.378 0 0112 8.443m-7.007 11.55A5.981 5.981 0 006.75 15.75v-1.5"/></svg>
                </div>
                <span class="text-sm font-bold tracking-wide text-text-primary">NUTRIMIND</span>
                <span class="rounded-full bg-info/20 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-info">Teacher</span>
            </div>

            {{-- Navigation --}}
            <nav class="mt-4 flex flex-1 flex-col gap-1 px-3">
                <a href="{{ route('teacher.class') }}"
                    class="flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium transition-colors {{ request()->routeIs('teacher.class') || request()->routeIs('teacher.dashboard') ? 'bg-surface-active text-text-primary' : 'text-text-secondary hover:bg-surface-elevated hover:text-text-primary' }}">
                ...
                <a href="{{ route('teacher.students.create') }}"
                    class="flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium transition-colors {{ request()->routeIs('teacher.students.create') ? 'bg-surface-active text-text-primary' : 'text-text-secondary hover:bg-surface-elevated hover:text-text-primary' }}">
                ...
                <a href="{{ route('teacher.classrooms.index') }}"
                    class="flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium transition-colors {{ request()->routeIs('teacher.classrooms.*') ? 'bg-surface-active text-text-primary' : 'text-text-secondary hover:bg-surface-elevated hover:text-text-primary' }}">
                ...
                <a href="{{ route('teacher.password.edit') }}"
                    class="flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium transition-colors {{ request()->routeIs('teacher.password.*') ? 'bg-surface-active text-text-primary' : 'text-text-secondary hover:bg-surface-elevated hover:text-text-primary' }}">
            ...
            <div class="border-t border-border-subtle px-3 py-4">
                <div class="mb-3 px-3">
                    ...
                    <p class="truncate text-xs text-text-muted">Teacher</p>
                </div>
